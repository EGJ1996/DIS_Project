...
void findPerformance(double swarm[swarmsize][datasize], double perf[swarmsize],
  ...
  /*
   Note that each particle corresponds to each robot
   */
  for (i = 0; i < swarmsize; i+=robots) {
    ...
    if (type == EVOLVE_AVG) {
      ...
      for (j=0;j<robots && i+j<swarmsize;j++) {
        // perf[i+j] is the performance of (i+j)th particle,age[i+j] is the age of (i+j)th particle, fit[j] is the fitness function as computed by each robot
