...
void reynolds_rules() {
           ...
           // printf("Called reynolds_rules from robot %d\n",robot_id);
        ...
        for (j=0;j<2;j++) 
	{
        ...
        printf("avg_speed[0] = %f,avg_speed[1] = %f\n",avg_speed[0],avg_speed[1]);
        printf("avg_loc[0] = %f,avg_loc[1] = %f\n",avg_loc[0],avg_loc[1]);
	/* Reynold's rules */
...
int main(){ 	
	...
	for(;;){
		...
		while (wb_receiver_get_queue_length(receiver) > 0 && count < FLOCK_SIZE) 
		{
			...
			printf("rob_nb = %d,robot_id = %d \n ",(int)rob_nb/FLOCK_SIZE,(int)robot_id/FLOCK_SIZE);
