...
double* pso(int n_swarmsize, int n_nb, double lweight, double nbweight, double vmax, double min, double max, int iterations, int n_datasize, int n_robots) {
  ...
  printf("Reached first checkpoint\n");
  ...
  for (i = 0; i < swarmsize; i++) {
    for (j = 0; j < datasize; j++) {
      ...
      // lbest is the local best, nbbest is the neighbourhood best
  ...
  for (k = 0; k < ITS_COEFF*iterations; k++) {
    ...
    // Updates the performance of a single particle
  ...
  for (i = 0; i < swarmsize; i+=robots) {
    // for (j=0;j<robots && i+j<swarmsize;j++) {
      // for (k=0;k<datasize;k++)
           // particles[j][k] = swarm[i+j][k];neighbors
    // }
    
    printf("Checkpoint 10\n");
    if (type == EVOLVE_AVG) {
	  // EvaluteEvalute current fitness
	  // fitness just calls the calc_fitness function which sends the weights and receives the fitness of the particles
      ...
      // for (j=0;j<robots && i+j<swarmsize;j++) {
          // perf[i+j] = age[i+j]*perf[i+j] + fit[j];
      // }
