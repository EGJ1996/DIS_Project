...
double* pso(int n_swarmsize, int n_nb, double lweight, double nbweight, double vmax, double min, double max, int iterations, int n_datasize, int n_robots,int opt_type) {
  printf("Started pso\n");
  ...
  findPerformance(swarm,perf,NULL,EVOLVE,robots,neighbors,opt_type);
  printf("Checkpoint 3\n");
  for (i = 0; i < swarmsize; i++) {
    lbestperf[i] = perf[i];
  ...
  findPerformance(lbest,lbestperf,NULL,SELECT,robots,neighbors,opt_type);
...
void findPerformance(double swarm[swarmsize][datasize], double perf[swarmsize],
		     ...
		     int neighbors[swarmsize][swarmsize],int opt_type) {
  ...
  for (i = 0; i < swarmsize; i+=robots) {
    ...
    if (type == EVOLVE_AVG) {
	  ...
	  if(opt_type == 0){
	     printf("Calling fitness function with evolve_avg type\n");
    	     fitness(particles,fit,neighbors);
    	     }
    	  else{
    	     printf("Calling fitness1 function with evolve_avg type\n");
      	     fitness1(particles,fit,neighbors);
    	  }
