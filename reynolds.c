...
#define STANDARD    -1
#define RAND_NB      0
#define DATASIZE1    6                  // Datasize for 2nd (3 weights + 2 recursive connections + 1 bias)
#define NCLOSE_NB    1
...
float migr[2];	                // Migration vector		// Buffer for the receiver node
int max_sens;
int braiten;
int i,j,k;
...
void reynolds_rules() {
           printf("Applying reynolds rules\n");
...
//        printf("id = %d, cx = %f, cy = %f\n", robot_id, cohesion[0], cohesion[1]);
        ...
        for (j=0;j<2;j++) 
	{
        ...
        printf("Finished reynolds_rules\n");
}

void braiten_reynolds(){
    
    printf("Inside braiten_reynolds of child\n");
    msl = 0; msr = 0;  
    max_sens = 0;
    bmsl = 0; bmsr = 0;
    sum_sensors = 0;
    ...
    for(i=0;i<NB_SENSORS;i++) 
    {
        ...
        // bmsr += good_w[i] * distances[i];
        // bmsl += good_w[i+NB_SENSORS] * distances[i];
        bmsr += good_w[i] * distances[i];
        bmsl += good_w[i+NB_SENSORS] * distances[i];
    }
    ...
    int count = 0;
    while (wb_receiver_get_queue_length(rec) > 0 && count < FLOCK_SIZE) 
    {
        printf("Received data from the supervisor\n");
        ...
        // sscanf(inbuffer,"%1d#%1d#%f#%f#%f",&main_state,&rob_nb,&rob_x,&rob_z,&rob_theta);
	...
	wb_motor_set_velocity(left_motor, msl_w);
           ...
           printf("Finished braiten_reynolds of child\n");
}
