...
void pso_braiten(){
  ...
  for (j=0;j<1;j++) {
    ...
    int optimization_type = 0; // type 0 for braitenberg weights optimization
...
void calc_fitness1(double weights[ROBOTS][DATASIZE1], double fit[ROBOTS], int its, int numRobs) {
  ...
  for (i=0;i<numRobs;i++) {
    ...
    buffer1[DATASIZE1+1] = its;
    // printf("Before sending data to the children\n");
    wb_emitter_send(emitter[i],(void *)buffer1,(DATASIZE1+2)*sizeof(double));
