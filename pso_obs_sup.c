...
#define ITS     500                       // Number of iterations to run
...
#define DATASIZE1    3
/* Fitness definitions */
#define FIT_ITS 200 // Number of fitness steps to run during evolution
...
void calc_fitness1(double weights[ROBOTS][DATASIZE1], double fit[ROBOTS], int its, int numRobs) {
   ...
   // It doesn't use the robots'  response
...
void pso_reynolds(){
  ...
  double f[MAX_ROB];			// Evaluated fitness (modified by calc_fitness() )
  double bestfit, bestw[DATASIZE1];	// Best fitness and weights
  ...
  for (j=0;j<10;j++) {
    ...
    // Get result of optimization
    int optimization_type = 1; // optimization type 1 for reynolds
    ...
    // Run FINALRUN tests and calculate average
    for (i=0;i<FINALRUNS;i+=MAX_ROB) {
      // printf("Inside finalruns loop iteration number %d,finalRuns = %d\n",i,FINALRUNS);
      printf("Before calling calc_fitness1\n");
      calc_fitness1(w,f,FIT_ITS,MAX_ROB);
      printf("After calling calc_fitness1\n");
  ...
  int buffsize = sizeof(buffer)/sizeof(double);
  for (i=0;i<ROBOTS;i++) {
    wb_emitter_send(emitter[i],(void *)buffer,(DATASIZE1+2)*sizeof(double));
  }
  printf("Finished executing pso_braiten of supervisor\n");
...
void braiten_reynolds(int migrx,int migrz){
	...
	if (t % 10 == 0) {
			...
			loc[i][2] = wb_supervisor_field_get_sf_rotation(robs_rotation[i])[3]; // THETA
                  		...
                  		// sprintf(buffer,"%1d#%1d#%f#%f#%f##%f#%f",1,i+offset,loc[i][0],loc[i][1],loc[i][2], migrx, migrz);
...
int main(int argc,char* args[]) {
  ...
  pso_braiten();
  // printf("Finished pso braiten in the supervisor\n");
  pso_reynolds();
...
// fitness1(double weights[ROBOTS][DATASIZE1], double fit[ROBOTS], int neighbors[SWARMSIZE][SWARMSIZE])
...
void calc_fitness(double weights[ROBOTS][DATASIZE], double fit[ROBOTS], int its, int numRobs) {
  ...
  // Not sure why the index is set to 1 when it clearly should be set to 0
  // buffer1[1] = 0.0;
